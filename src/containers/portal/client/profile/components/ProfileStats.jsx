...
function ProfileStats() {
  ...
  const {
    userInfo: {
      ...
      total_job_posted: totalJobs = 0,
    },
  } = useSelector(state => state.auth);

  // constants
  const userFullName = Boolean(firstName) && Boolean(lastName) ? `${firstName} ${lastName}` : username;

  const handleClick = to => {
    if (to === 'notfications') {
      window.scrollTo(0, notification.current.offsetTop - 100);
    }
    if (to === 'billing') {
      window.scrollTo(0, billing.current.offsetTop - 100);
    }
    if (to === 'security') {
      window.scrollTo(0, security.current.offsetTop - 100);
    }
  };
  ...
  return (
    <>
      ...
      <BasicInfo />
      <Notifications innerRef={notification} id="notifications" />
      <ConnectedAccounts />
      <BillingPayments innerRef={billing} />
      <BillingAddress />
      <Security innerRef={security} />
      <DeactivateAccount />
    </>
  );
}

...
function JobPostingFormUploadedFiles({ name }) {
  // REFERENCING HOOKS
  ...
  return (
    <FieldArray name={name}>
      {({ remove }) => (
        <Box className="col-12 col-md-4 col-lg-8 mt-2 d-flex flex-wrap my-2 ms-auto">
                ...
                <Box key={item.name} className={`${styles.uploadedFile} p-2 me-2 mt-2`}>
                  <Box className="d-flex justify-content-between">
                    <Box className="d-flex">
                      <img src={setIconByFileType(item?.name || item?.type)} alt="file" />
...
JobPostingFormUploadedFiles.propTypes = {
  name: propTypes.string.isRequired,
};

export default JobPostingFormUploadedFiles;

...
import { useUpdateSkillMutation } from 'services/private/jobPost';
...
function UpdateSkillModal({ isOpen, handleToggle, selected }) {
  // REFERENCING HOOKS
  ...
  const [
    updateSkill,
    { isLoading: updateSkillLoading, error: updateSkillError, isSuccess: updateSkillSuccess },
  ] = useUpdateSkillMutation();

  // CUSTOM ERROR HOOKS
  useHandleApiResponse(updateSkillError, updateSkillSuccess, 'Skill has been Updated');
  ...
  return (
    <Modal open={isOpen || false} onClose={handleToggle}>
      <Box sx={jobPostingSkillModalBoxStyles}>
        <Formik
          enableReinitialize
          initialValues={initValues}
          onSubmit={async submittedValues => {
            const payload = { ...submittedValues, id: selected?.id };
                ...
                <Box className="d-flex align-items-center justify-content-between px-4 py-3">
                  ...
                  <Stack spacing={2} direction="row">
                    ...
                    <Button
                      ...
                      startIcon={updateSkillLoading && <CircularProgress size={16} />}
                    >
                      Save
                    </Button>
                  </Stack>
